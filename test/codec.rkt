...
(define (int-segment-decode! len bv pos v.target i.target)
  (define (int-segment-decode/frame-of-reference x.min byte-width pos)
    (let ((end (unsafe-fx+ i.target len)))
      (let ((go (lambda (n-ref)
                  (let loop ((i i.target) (pos pos))
                    (cond ((unsafe-fx< i end)
                           (unsafe-vector*-set! v.target i (unsafe-fx+ (n-ref bv pos) x.min))
  ...
  (let* ((type (unsafe-bytes-ref bv pos))
    ...
    (cond
      ...
      ;        (pos        (unsafe-fx+ pos 1))
      ;        (x.min      (byte-width->int-min byte-width)))
      ;   (int-segment-decode/frame-of-reference x.min byte-width pos)))
      ((? compression-type.int:frame-of-reference)
       (let* ((byte-width (unsafe-bytes-ref bv pos))
              (pos        (unsafe-fx+ pos 1))
              (x.min      (unsafe-fx+ (unsafe-bytes-nat-ref byte-width bv pos)
      ...
      ((? compression-type.int:single-value)
       (let* ((byte-width (unsafe-bytes-ref bv pos))
              (pos        (unsafe-fx+ pos 1))
              (x.sole     (unsafe-fx+ (unsafe-bytes-nat-ref byte-width bv pos)
                                      (byte-width->int-min byte-width)))
              (pos        (unsafe-fx+ pos byte-width))
              (end        (unsafe-fx+ i.target len)))
         (let loop ((i i.target))
           (when (unsafe-fx< i end)
             (unsafe-vector*-set! v.target i x.sole)
             (loop (unsafe-fx+ i 1))))
         pos))
      ((? compression-type.int:delta-single-value)
       (let* ((byte-width (unsafe-bytes-ref bv pos))
              (pos        (unsafe-fx+ pos 1))
              (x.start    (unsafe-fx+ (unsafe-bytes-nat-ref byte-width bv pos)
                                      (byte-width->int-min byte-width)))
              (pos        (unsafe-fx+ pos byte-width))
              (byte-width (unsafe-bytes-ref bv pos))
              (pos        (unsafe-fx+ pos 1))
              (x.delta    (unsafe-fx+ (unsafe-bytes-nat-ref byte-width bv pos)
                                      (byte-width->int-min byte-width)))
              (pos        (unsafe-fx+ pos byte-width))
              (end        (unsafe-fx+ i.target len)))
         (unsafe-vector*-set! v.target i.target x.start)
         (let loop ((i (unsafe-fx+ i.target 1)) (x.prev x.start))
           (when (unsafe-fx< i end)
             (let ((v (unsafe-fx+ x.prev x.delta)))
               (unsafe-vector*-set! v.target i v)
...
(define (int-encode! bv pos v)
  (let ((byte-width (nat-min-byte-width (if (unsafe-fx< v 0)
                                            (unsafe-fx- (unsafe-fx* (unsafe-fx- v) 2) 1)
                                            v))))
    (unsafe-bytes-set! bv pos byte-width)
    (unsafe-bytes-nat-set! byte-width bv (unsafe-fx+ pos 1)
                           (unsafe-fx- v (byte-width->int-min byte-width)))
    (unsafe-fx+ pos 1 byte-width)))

;; Returns the pos immediately following segment
(define (int-segment-encode!/frame-of-reference x.min width len bv pos v.source i.source)
  (unsafe-bytes-set! bv pos compression-type.int:frame-of-reference)
  (let ((pos (int-encode! bv (unsafe-fx+ pos 1) x.min))
        (end (unsafe-fx+ i.source len)))
    (unsafe-bytes-set! bv pos width)
    (let ((go (lambda (n-set!)
                (let loop ((i i.source) (pos (unsafe-fx+ pos 1)))
                  (cond ((unsafe-fx< i end)
                         (n-set! bv pos (unsafe-fx- (unsafe-vector*-ref v.source i) x.min))
...
;; Returns the pos immediately following segment
(define (int-segment-encode!/single-value x.sole bv pos)
  (unsafe-bytes-set! bv pos compression-type.int:single-value)
  (int-encode! bv (unsafe-fx+ pos 1) x.sole))

;; Returns the pos immediately following segment
(define (int-segment-encode!/delta-single-value x.start x.delta bv pos)
  (unsafe-bytes-set! bv pos compression-type.int:delta-single-value)
  (let ((pos (int-encode! bv (unsafe-fx+ pos 1) x.start)))
    (int-encode! bv pos x.delta)))
