    ...
    (cond
      ...
      ((? compression-type.int:dictionary)
       (let* ((len.dict (unsafe-bytes-ref bv pos))
              (pos      (unsafe-fx+ pos 1))
              (x*       (make-vector len.dict))
              (pos      (int-segment-decode! len.dict bv pos x* 0)))
         (let loop ((j 0))
           (when (unsafe-fx< j len)
             (let* ((i   (unsafe-fx+ i.target j))
                    (pos (unsafe-fx+ pos      j))
                    (x   (unsafe-vector*-ref x* (unsafe-bytes-ref bv pos))))
               (unsafe-vector*-set! v.target i x)
               (loop (unsafe-fx+ j 1)))))
         (unsafe-fx+ pos len)))
...
;; Returns the pos immediately following segment
(define (int-segment-encode!/dictionary x* len bv pos v.source i.source)
  (unsafe-bytes-set! bv pos compression-type.int:dictionary)
  (let* ((pos      (unsafe-fx+ pos 1))
         (len.dict (unsafe-vector*-length x*))
         (pos      (begin (unsafe-bytes-set! bv pos len.dict) (unsafe-fx+ pos 1)))
         (x0       (unsafe-vector*-ref x* 0))
         (pos      (let loop ((i 1) (x.min x0) (x.max x0))
                     (if (unsafe-fx< i len.dict)
                         (let ((x (unsafe-vector*-ref x* i)))
                           (cond ((unsafe-fx< x x.min) (loop (unsafe-fx+ i 1) x     x.max))
                                 ((unsafe-fx< x.max x) (loop (unsafe-fx+ i 1) x.min x))
                                 (else                 (loop (unsafe-fx+ i 1) x.min x.max))))
                         (let ((width (nat-min-byte-width (unsafe-fx- x.max x.min))))
                           (int-segment-encode!/frame-of-reference
                             x.min width len.dict bv pos x* 0)))))
         (x=>i     (let loop ((i 0) (x=>i (hash)))
                     (if (unsafe-fx< i len.dict)
                         (loop (unsafe-fx+ i 1) (hash-set x=>i (unsafe-vector*-ref x* i) i))
                         x=>i))))
    (let loop ((j 0))
      (when (unsafe-fx< j len)
        (let ((i   (unsafe-fx+ i.source j))
              (pos (unsafe-fx+ pos j)))
          (unsafe-bytes-set! bv pos (hash-ref x=>i (unsafe-vector*-ref v.source i)))
          (loop (unsafe-fx+ j 1)))))
    (unsafe-fx+ pos len)))
