      ...
      ((? compression-type.int:delta)
       (let* ((byte-width (unsafe-bytes-ref bv pos))
         ...
         (let* ((len (unsafe-fx- len 1))
                ...
                (end (unsafe-fx+ i len))
                (pos (int-segment-decode! len bv pos v.target i)))
           (let loop ((i i) (v.prev v.start))
             (when (unsafe-fx< i end)
               (let ((v (unsafe-fx+ (unsafe-vector*-ref v.target i) v.prev)))
...
;; Returns the pos immediately following segment
(define (int-segment-encode!/frame-of-reference v.min width len bv pos v.source i.source)
  (unsafe-bytes-set! bv pos compression-type.int:frame-of-reference)
  (let ((pos (int-encode! bv (unsafe-fx+ pos 1) v.min))
        (end (unsafe-fx+ i.source len)))
    (unsafe-bytes-set! bv pos width)
    (let ((go (lambda (n-set!)
                (let loop ((i i.source) (pos (unsafe-fx+ pos 1)))
                  (cond ((unsafe-fx< i end)
                         (n-set! bv pos (unsafe-fx- (unsafe-vector*-ref v.source i) v.min))
