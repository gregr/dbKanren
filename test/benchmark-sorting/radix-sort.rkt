...
(define (vector-radix-sort! v t->key key-byte-count (start 0) (end (vector-length v)))
  ...
  (let loop ((shift 0))
    ...
    (let loop.all ((k 0) (j start))
      (when (< k count.parts)
        (let ((parts.k     (vector-ref parts     k))
              (pos.parts.k (vector-ref pos.parts k))
              (buf.k       (vector-ref buf       k))
              (pos.k       (vector-ref pos       k)))
          (let loop ((i 0) (j j))
            (if (< i pos.parts.k)
              (let ((part (vector-ref parts.k i)))
                (let loop ((i 0))
                  (when (< i count.buffer)
                    (vector-set! v (+ j i) (vector-ref part i))
                    (loop (+ i 1))))
                (loop (+ i 1) (+ j count.buffer)))
              (let loop ((i 0))
                (if (= i pos.k)
                  (loop.all (+ k 1) (+ j pos.k))
                  (begin (vector-set! v (+ j i) (vector-ref buf.k i))
                         (loop (+ i 1))))))))))
    (when (< shift shift.final)
      (loop (+ shift size.shift)))))
             ...
             (let loop ((i 0))
               (if (= i pos.parts.i)
                 (begin ((unsafe-vector->enumerator buf.i 0 pos.i) k!)
                        (free-buffer buf.i))
                 (begin (let ((part (vector-ref parts.i i)))
