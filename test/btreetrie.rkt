...
(define (structure-compile-read S E.buffer E.limit.left E.limit.right E.k.fail E*.succeed)
  #`(let ((k.fail   #,E.k.fail)
          (bytecount->n-ref
            (lambda (bytecount)
              (case bytecount
                ...
                (syntax-case S* (struct size-= size-min size-max byte bytes text array
                  ...
                  ((((bytes E.count) name ...) rest ...)
                   #`(let* ((count E.count)
                            ...
                            (value ((bytecount->n-ref count) cur)))
                       (let ((name value) ...) #,(loop #'(rest ...)))))
                  ((((array E.count E.len) name ...) rest ...)
                   #`(let* ((count E.count)
                            (len   E.len)
                            (cur   (unsafe-fx- cur (unsafe-fx* count len)))
                            (value (let ((n-ref (bytecount->n-ref count))
                                         (n*    (make-fxvector len 0)))
                                     (let loop ((i 0) (pos pos))
                                       (when (unsafe-fx< i len)
                                         (fxvector-set! n* i (n-ref pos))
                                         (loop (unsafe-fx+ i 1) (unsafe-fx+ pos count))))
                                     n*)))
                       (let ((name value) ...) #,(loop #'(rest ...)))))
                  ((((text E.len) name ...) rest ...)
                   #`(let* ((len   E.len)
                            (next  (unsafe-fx- cur len))
                            (value (subbytes buffer next cur))
                            (cur   next))
                       (let ((name value) ...) #,(loop #'(rest ...)))))
...
(pretty-result
  (syntax->datum
    (structure-compile-read
      stx.btt-node #'E.buffer #'E.limit.left #'E.limit.right #'E.k.fail (list #'E.succeed))))
