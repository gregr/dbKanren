...
(define (fxvector-copy! vec.out out.start vec.in in.start in.end)
      ...
      (let ((row.scaled (unsafe-fx* row count.cols)))
        (let loop.col ((col (unsafe-fx- count.cols 1)))
          (when (unsafe-fx<= 0 col)
            (fxvector-set! vec.rows (unsafe-fx+ row.scaled col)
                           (fxvector-ref vec.cols (unsafe-fx+ (unsafe-fx* col count.rows) row)))
  ...
  (let loop.row ((row (unsafe-fx- count.rows 1)))
    (when (unsafe-fx<= 0 row)
      (let ((row.scaled (unsafe-fx* row count.cols)))
        (let loop.col ((col (unsafe-fx- count.cols 1)))
          (when (unsafe-fx<= 0 col)
            (fxvector-set! vec.cols (unsafe-fx+ (unsafe-fx* col count.rows) row)
                           (fxvector-ref vec.rows (unsafe-fx+ row.scaled col)))
            (loop.col (unsafe-fx- col 1))))
        (loop.row (unsafe-fx- row 1))))))

(define (row-merge-sort! vec.rows vec.buffer count.cols count.rows)
  (define (tuple<? a.rows b.buffer)
    (let ((x.a (fxvector-ref vec.rows   a.rows))
          (x.b (fxvector-ref vec.buffer b.buffer)))
      (or (unsafe-fx< x.a x.b)
          (and (unsafe-fx= x.a x.b)
               (let loop ((i 1))
                 (and (unsafe-fx< i count.cols)
                      (let ((x.a (fxvector-ref vec.rows   (unsafe-fx+ a.rows   i)))
                            (x.b (fxvector-ref vec.buffer (unsafe-fx+ b.buffer i))))
                        (or (unsafe-fx< x.a x.b)
                            (and (unsafe-fx= x.a x.b)
                                 (loop (unsafe-fx+ i 1)))))))))))
      ...
      (when (unsafe-fx< 1 diff)
        (let ((mid (unsafe-fx+ start (unsafe-fxrshift diff 1))))
          ...
          (let ((start (unsafe-fx* count.cols start))
            ...
            (fxvector-copy! vec.buffer start vec.rows start mid)
            (let merge! ((in.buffer start) (in.rows mid) (out.rows start))
              (if (tuple<? in.rows in.buffer)
                (let ((in.next.rows (unsafe-fx+ in.rows count.cols)))
                  (fxvector-copy! vec.rows out.rows vec.rows in.rows in.next.rows)
                  (if (unsafe-fx< in.next.rows end)
                    (merge! in.buffer in.next.rows (unsafe-fx+ out.rows count.cols))
                    (fxvector-copy! vec.rows (unsafe-fx+ out.rows count.cols) vec.buffer in.buffer mid)))
                (let ((in.next.buffer (unsafe-fx+ in.buffer count.cols)))
                  (fxvector-copy! vec.rows out.rows vec.buffer in.buffer in.next.buffer)
...
(define (row-deduplicate! vec.rows count.cols count.rows)
  (define (tuple=? a b)
    (let ((x.a (fxvector-ref vec.rows a))
          (x.b (fxvector-ref vec.rows b)))
      (and (unsafe-fx= x.a x.b)
           (let loop ((i 1))
             (and (unsafe-fx< i count.cols)
                  (let ((x.a (fxvector-ref vec.rows (unsafe-fx+ a i)))
                        (x.b (fxvector-ref vec.rows (unsafe-fx+ b i))))
                    (and (unsafe-fx= x.a x.b)
                         (loop (unsafe-fx+ i 1)))))))))
  (unsafe-fxquotient
                  ...
                  (if (unsafe-fx< current end)
                    (if (tuple=? prev current)
                      ...
                      (let ((prev (unsafe-fx+ prev    count.cols))
                        ...
                        (fxvector-copy! vec.rows prev vec.rows current next)
                        (loop.gap prev next)))
