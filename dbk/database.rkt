                 ...
                 (and (unsafe-fx< i count.cols)
                      (let ((x.a (unsafe-fxvector-ref vec.a (unsafe-fx+ a i)))
                        ...
                        (or (unsafe-fx< x.a x.b)
                            (and (unsafe-fx= x.a x.b)
                                 (loop (unsafe-fx+ i 1)))))))))))
  ;; TODO: check for reverse-sorted runs?
  (let sort-range! ((start 0) (start.unsorted 0) (end count.rows))
    (let ((start.unsorted
            (if (unsafe-fx<= start.unsorted start)
              (let ((i   (unsafe-fx+ start.rows (unsafe-fx* count.cols start)))
