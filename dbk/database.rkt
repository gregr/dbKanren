       ...
       (define (text->id bs)
         (or (hash-ref text=>id bs #f)
             (let ((id (hash-count text=>id)))
               (set! text=>id (hash-set text=>id bs id))
...
(define (bytes-compare a b)
  (let* ((len.a (bytes-length a)) (len.b (bytes-length b)) (end (min len.a len.b)))
    (let loop ((i 0))
      (if (= i end)
          (cond ((< len.a len.b) -1)
                ((< len.b len.a)  1)
                (else             0))
          (let ((x.a (bytes-ref a i)) (x.b (bytes-ref b i)))
            (cond ((< x.a x.b) -1)
                  ((< x.b x.a)  1)
                  (else        (loop (+ i 1)))))))))

(define (btree-count bt) (vector-ref bt 0))
...
(define (btree-2?      t) (= (vector-length t) 4))
(define (btree-2-key   t) (vector-ref t 0))
(define (btree-2-leaf  t) (vector-ref t 1))
(define (btree-2-left  t) (vector-ref t btree-2-left:i))
(define (btree-2-right t) (vector-ref t btree-2-right:i))

(define (btree-2-left-set!  t u) (vector-set! t btree-2-left:i  u))
(define (btree-2-right-set! t u) (vector-set! t btree-2-right:i u))

(define btree-2-left:i  2)
(define btree-2-right:i 3)

(define (btree-3-left-key   t) (vector-ref t 0))
(define (btree-3-right-key  t) (vector-ref t 1))
(define (btree-3-left-leaf  t) (vector-ref t 2))
(define (btree-3-right-leaf t) (vector-ref t 3))
(define (btree-3-left       t) (vector-ref t btree-3-left:i))
(define (btree-3-middle     t) (vector-ref t btree-3-middle:i))
(define (btree-3-right      t) (vector-ref t btree-3-right:i))

(define (btree-3-left-set!   t u) (vector-set! t btree-3-left:i   u))
(define (btree-3-middle-set! t u) (vector-set! t btree-3-middle:i u))
(define (btree-3-right-set!  t u) (vector-set! t btree-3-right:i  u))
...
(define (btree-ref-or-set! bt x)
  (let loop ((t        (vector-ref bt 1))
             (replace! (lambda (u)            (vector-set! bt 1 u)))
             (expand!  (lambda (key leaf l r) (vector-set! bt 1 (vector key leaf l r)))))
    (cond
      ((not t) (let ((count (vector-ref bt 0)))
                 (vector-set! bt 0 (+ count 1))
                 (expand! x count #f #f)
                 count))
