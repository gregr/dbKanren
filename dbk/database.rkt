         ...
         (let ((count.tuples.unique (let ((count.rows (quotient i.tuple count.cols)))
                                      (performance-log
                                        ...
                                        (unsafe-fxquotient (row-deduplicate! vec.rows 0 count.cols count.rows)
                                                           count.cols)))))
           (time (transpose-row-to-col! vec.cols vec.rows 0 count.cols count.tuples.unique))
          ...
          (for-each
            (lambda (tid)
              ...
              (unless (has-index-prefix? (car prefixes))
                ...
                (time (transpose-col-to-row! vec.cols vec.rows 0 count.cols count.rows))
                (performance-log
                  `(sorting ,count.rows tuples)
                  (row-merge-sort! vec.rows vec.cols 0 count.cols count.rows))
                (time (transpose-row-to-col! vec.cols vec.rows 0 count.cols count.rows))
    ...
    (let ((diff (unsafe-fx- end start)))
      (when (unsafe-fx< 1 diff)
        (let ((mid (unsafe-fx+ start (unsafe-fxrshift diff 1))))
          ...
          (sort-range! mid   end)
          (let ((start (unsafe-fx+ start.rows (unsafe-fx* count.cols start)))
                (mid   (unsafe-fx+ start.rows (unsafe-fx* count.cols mid)))
                (end   (unsafe-fx+ start.rows (unsafe-fx* count.cols end))))
