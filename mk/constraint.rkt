...
(define (state-schedule-update st x vcx.new)
  (define xcx (state-var=>cx-ref st x))
  (if (vcx? xcx)
    (state-schedule-mvcx-new st x vcx.new)
    (state-schedule-mvcx st xcx vcx.new)))
(define (state-schedule-mvcx-new st x xcx)
  (let* ((m (mvcx #t x xcx))
         (q (state-pending st))
         (q (queue (cons m (queue-mvcxs q)) (cons x (queue-high q))
                   (queue-low q))))
    (state (hash-set (state-var=>cx st) x m) (state-store st)
           (state-tables st) (state-disjs st) (state-uses st) q)))
...
(define (state-pending-clear st)
  (let ((st (foldl (lambda (m st) (if (not (mvcx-var m)) st
                                    (state-var=>cx-set
                                      st (mvcx-var m) (mvcx-vcx m))))
                   st (queue-mvcxs (state-pending st)))))
    ;; TODO: assert that pending is empty
    (state (state-var=>cx st) (state-store st) (state-tables st)
           (state-disjs st) (state-uses st) queue.empty)))

(define (state-pending-push-high st x)
  (let* ((q (state-pending st))
         (q (queue (queue-mvcxs q) (cons x (queue-high q)) (queue-low q))))
    (state (state-var=>cx st) (state-store st) (state-tables st)
           (state-disjs st) (state-uses st) q)))
(define (state-pending-push-low st x)
  (let* ((q (state-pending st))
         (q (queue (queue-mvcxs q) (queue-high q) (cons x (queue-low q)))))
    (state (state-var=>cx st) (state-store st) (state-tables st)
           (state-disjs st) (state-uses st) q)))
(define (state-pending-pop st)
  ...
  (define (state/pending high low)
    (state (state-var=>cx st) (state-store st) (state-tables st)
           (state-disjs st) (state-uses st) (queue (queue-mvcxs q) high low)))
