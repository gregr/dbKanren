    ...
    (let ((t (walk* st t)))
      (cond ((ground? t) (and ((if (bounds-lb-inclusive? b) any<=? any<?)
            ...
            (else (define xcx     (state-var=>cx-ref st t))
                  ...
                  (define lbi?
                    (if (and (not lb?)
                             (or (equal? (bounds-lb-inclusive? b)
                                         (bounds-lb-inclusive? b.0))
                                 (not (equal? (bounds-lb b.0) (bounds-lb b)))))
                      #f (bounds-lb-inclusive? b)))
                  (define ubi?
                    (if (and (not ub?)
                             (or (equal? (bounds-ub-inclusive? b)
                                         (bounds-ub-inclusive? b.0))
                                 (not (equal? (bounds-ub b.0) (bounds-ub b)))))
                      #f (bounds-ub-inclusive? b)))
                  (if (or lb? ub? lbi? ubi?)
                    (let ((b.new (bounds (if lb? (bounds-lb b) (bounds-lb b.0))
                                         (if lbi? (bounds-lb-inclusive? b)
                                           (bounds-lb-inclusive? b.0))
                                         (if ub? (bounds-ub b) (bounds-ub b.0))
                                         (if ubi? (bounds-ub-inclusive? b)
                                           (bounds-ub-inclusive? b.0)))))
                      (define vcx.new (vcx-bounds-set vcx.old b.new))
                      (if (vcx? xcx) (state-schedule-mvcx-new st t vcx.new)
...
(define ((mvcx-update m) st)
  ...
  (let loop ((dcxs (reverse (vcx-domain xcx))) (dcxs.seen '()) (b b.0))
    (cond ((null? dcxs)
           (if (eq? b b.0) (begin (set-mvcx-pending?! m #f) st)
             (let loop ((acxs (vcx-arc xcx)) (acxs.seen '()) (st st))
               (if (null? acxs)
                 (begin (set-mvcx-vcx! m (vcx-update b dcxs.seen acxs.seen))
                        (state-pending-push-low st (mvcx-update m)))
                 (let ((acx (car acxs)))
                   (match (acx 'update st b)
                     (#f                #f)
                     (`(succeed  . ,st) (loop (cdr acxs) acxs.seen st))
                     (`(continue . ,st) (loop (cdr acxs) (cons acx acxs.seen)
                                              st))))))))
          (else
            (define dcx (car dcxs))
            (match (dcx 'update b)
              (#f       #f)
              ('succeed (loop (cdr dcxs) dcxs.seen b))
              (`(assign . ,value)
                (set-mvcx-vcx! m (vcx-update bounds.any (append dcxs dcxs.seen)
                                             (vcx-arc xcx)))
                (assign st x value))
...
(define (state-schedule-mvcx-new st x xcx)
  ...
  (define m (mvcx #t x (vcx-update (vcx-bounds xcx)
                                   (map load (vcx-domain xcx))
                                   (map load (vcx-arc xcx)))))
...
(define (state-mvcxs-clear st)
  ...
  (let ((st (foldl (lambda (m st)
                     ...
                     (if x (match-let*
                       ...
                       st))
                   st (state-mvcxs st))))
    ;; TODO: assert that pending.high and pending.low are empty
    (state (state-var=>cx st) (state-store st) (state-tables st)
