...
(define (vcx-domain-add x cx) (vcx:set x (domain (set-add (vcx-domain x) cx))))
...
(define (vcx-==/use-add x u)  (vcx:set x (==/use (cons u (vcx-==/use x)))))
...
(define (state-store-set st k v)
  (state:set st (store (hash-set (state-store st) k v))))
  ...
  (let* ((q      (state-pending st))
         ...
         (q      (if (set-member? recent x)
                   ...
                   (queue (set-add recent x) (cons x (queue-high q))
                          (queue-low q)))))
    (state:set st (pending q))))
(define (state-pending-pop st)
  (define q (state-pending st))
  (define (state/pending high low)
    (state:set st (pending (queue (queue-recent q) high low))))
...
(define (state-pending-clear st)
  (state:set st (pending queue.empty)))
