#lang racket/base
(provide path->format file-stats produce/pop-header
...
(require "codec.rkt" "enumerator.rkt" "misc.rkt"
...
(define ((pop-header ? expected en) k!)
  (define header-seen? #f)
  (en (lambda (x) (cond (header-seen? (k! x))
                        (else (unless (? x)
...
(define (produce/pop-header produce header)
  (define (produce/pop ? h) (pop-header ? h produce))
  (match header
    (#f produce)
...
(define (in:transform  produce f)      (cond (f    (lambda (k!) (produce (lambda (x) (k! (f x))))))
                                             (else produce)))
...
(define (port-produce in close? format type)
  (case format
    ((json) (define data (json:read in))
            (when close? (close?))
            (vector->enumerator data))
    (else   (define get
              (case format
...
(define (port-consume out close? format type en)
  (case format
    ((json) (json:write out (list->vector (enumerator->list en)))
            (when close? (close?)))
    (else   (en (case format
