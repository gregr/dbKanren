...
;; start end -> (yield! x start.span end.span)
(define (column-group             col) (col 'group))
;; key* -> start end -> (yield! key start.span end.span)
(define (column-group/key*        col) (col 'group/key*))
;; key* -> start end -> (yield! key start.insertion-and-span end.span)
;; - with sorted-group/key*, if a key is not present, it will still be yielded, but its
;;   start.span and end.span will be equal
...
(define (unsafe-fxvector-sort!/<? <? z* start end)
  (define (i<i? i j) (<? (unsafe-fxvector-ref z* i) (unsafe-fxvector-ref z* j)))
  (define (find-unsorted start end)
    (let loop ((i start))
      (let ((i+1 (unsafe-fx+ i 1)))
        (if (or (unsafe-fx= i+1 end) (i<i? i+1 i)) i+1 (loop i+1)))))
        ...
        (let loop ((i 0) (start start))
          (when (unsafe-fx< i len)
            (let* ((key (unsafe-vector*-ref key* i)))
              (let* ((start.span (unsafe-bisect-skip start      end (lambda (j) (i<x? j key))))
                     (end.span   (unsafe-bisect-skip start.span end (lambda (j) (i=x? j key)))))
                (yield! key start.span end.span)
                (loop (unsafe-fx+ i 1) end.span)))))))))
    ...
    (cond
      ...
      ((? encoding.int:run-single-length)
       ...
       (let* ((count.run    (unsafe-fx+ (unsafe-bytes-nat-ref/width byte-width bv pos.bv) 1))
              (pos.bv       (unsafe-fx+ pos.bv byte-width)))
         (let*-values (((col.run pos.bv) (column:encoding.int&end bv pos.bv count.run)))
           (values (column:encoding.int:run-single-length col.run count.run count)
                   pos.bv))))
      ((? encoding.int:single-value)
       (let ((single-value (unsafe-bytes-int-ref/width byte-width bv pos.bv)))
    ...
    (cond
      ...
      ((? encoding.text:raw-single-length)
       (let* ((len        (unsafe-bytes-nat-ref/width byte-width bv pos.bv))
              (pos.bv     (unsafe-fx+ pos.bv byte-width)))
         (values (column:encoding.text:raw-single-length len bv pos.bv count)
      ...
      ((? encoding.text:run-single-length)
       ...
       (let* ((count.run    (unsafe-fx+ (unsafe-bytes-nat-ref/width byte-width bv pos.bv) 1))
              (pos.bv       (unsafe-fx+ pos.bv byte-width)))
         (let*-values (((col.run pos.bv) (column:encoding.text&end bv pos.bv count.run)))
           (values (column:encoding.text:run-single-length col.run count.run count)
                   pos.bv))))
      ((? encoding.text:single-value)
...
(define (column:encoding.int:nat byte-width bv start.bv count)
  ;; TODO: maybe specialize ref for z.min=0
...
(define (column:encoding.text:raw col.offset bv start.bv count)
  (let ((ref.offset (column-ref col.offset)))
    (define (describe)
      (let ((size.bv (- (ref.offset count) start.bv)))
        `(column:encoding.text:raw
           (count                     ,count)
           (byte-size                 ,size.bv)
           (average-bytes-per-element ,(exact->inexact (/ size.bv count)))
           (column.offset             ,(column-describe col.offset)))))
    (define (ref i)
      (let* ((start (unsafe-fx+ (ref.offset i) start.bv))
             (end   (unsafe-fx+ (ref.offset (unsafe-fx+ i 1)) start.bv))
             (count (unsafe-fx- end start))
             (t     (make-bytes count)))
        (unsafe-bytes-copy! t 0 bv start end)
        t))
    (define (i=x? i x)
      (let* ((start (ref.offset i))
             (end   (ref.offset (unsafe-fx+ i 1)))
    ...
    (define (i<x? i x)
      (let* ((start (ref.offset i))
             (len.i (unsafe-fx- (ref.offset (unsafe-fx+ i 1)) start))
    ...
    (controller:column/array ref unsafe-bytes=? i=x? i<x? 'column:encoding.text:raw count
                             describe)))
(define (column:encoding.text:raw-single-length len bv start.bv count)
  ;; TODO: maybe specialize ref for fixed length offsets
  (let ((col.offset (column:encoding.int:delta-single-value 0 len (unsafe-fx+ count 1))))
    (column:encoding.text:raw col.offset bv start.bv count)))
...
(define (column:encoding.X:run-length/X column-type-id.description col.offset col.run count.run count)
  (define (describe)
    `(,column-type-id.description
       (count         ,count)
       (column.offset ,(column-describe col.offset))
      ...
      (let* ((i.start (if (unsafe-fx= start 0)
                          ...
                          (unsafe-fx-
                            (unsafe-bisect-skip 0 (unsafe-fx+ count.run 1)
                                                (lambda (i) (unsafe-fx<= (ref.offset i) start)))
                            1)))
             (i.end   (if (unsafe-fx= end count)
                          count.run
                          (unsafe-bisect-skip i.start (unsafe-fx+ count.run 1)
                                              (lambda (i) (unsafe-fx< (ref.offset i) end)))))
             (enum    (group.run i.start i.end)))
        (lambda (yield!)
          (enum (lambda (x start.run end.run) (yield! x (unsafe-fxmax (ref.offset start.run) start)
                                                      (unsafe-fxmin (ref.offset end.run) end)))))))
...
(define (column:encoding.int:run-length col.offset col.run count.run count)
  (column:encoding.X:run-length/X
    'column:encoding.int:run-length col.offset col.run count.run count))
(define (column:encoding.text:run-length col.offset col.run count.run count)
  (column:encoding.X:run-length/X
    'column:encoding.text:run-length col.offset col.run count.run count))
...
(define (column:encoding.int:run-single-length col.run count.run count)
  (column:encoding.X:run-single-length/X
    'column:encoding.int:run-single-length col.run count.run count))
(define (column:encoding.text:run-single-length col.run count.run count)
  (column:encoding.X:run-single-length/X
    'column:encoding.text:run-single-length col.run count.run count))
...
(define (parser:tsv:lf in count.columns batch-size code*)
  ...
  (let* ((len.code*.max  (fxvector-length code*))
         ...
         (len.buffer.tsv batch-size)
         (buffer.tsv     (make-bytes len.buffer.tsv))
         (pos.free       0))
    (lambda (bt)  ; Return number of rows and number of bytes processed.
      (define (parse pos.end)
        (verbose-log `(parsing ,pos.end bytes))
        (let loop.row ((pos.start.row 0) (row 0) (i 0))
          (define (suspend)
            ...
            (verbose-log `(pausing with ,pos.free bytes remaining))
            (values row pos.start.row))
                   ...
                   (let ((x (subbytes buffer.tsv pos.start.col pos)))
                     (unless (unsafe-fx= remaining-col 1)
                       (error "end-of-file after too few columns"
                              ...
                              x))
                     (unsafe-fxvector-set! code* i (btree-ref-or-set! bt x))
                     (values (unsafe-fx+ row 1) pos.end)))
                  ((unsafe-fx= row 0) (error "no rows parsed before end-of-file"
                                             (- (file-position in) pos.end)))
                  (else (suspend))))
              (suspend))))
      (let ((count.read (performance-log `((reading ,(unsafe-fx- len.buffer.tsv pos.free) bytes)
                                           (at ,pos.free))
                                         (read-bytes! buffer.tsv in pos.free len.buffer.tsv))))
        (if (eof-object? count.read)
            (begin (verbose-log '(end-of-file: no new bytes read))
                   (if (= pos.free 0) eof (parse pos.free)))
            (begin (verbose-log `(read ,count.read bytes))
...
(define ((build-tables/parser parser) in out column-permutation (input-batch-size (expt 2 28))
  ...
  (let* ((column-permutation
           (cond ((fxvector? column-permutation) column-permutation)
                 ((vector?   column-permutation) (apply fxvector (vector->list column-permutation)))
                 ((list?     column-permutation) (apply fxvector column-permutation))
                 (else (error "not a column permutation" column-permutation))))
         (count.row-groups.max (max 1 (quotient input-batch-size row-group-size)))
         (count.columns        (fxvector-length column-permutation))
         (column-type*         (make-fxvector count.columns type-code.text))
         (len.code*.max        (quotient input-batch-size 16))
         (len.row*.max         (quotient len.code*.max count.columns))
         (len.code*.max        (* len.row*.max count.columns))
         (len.row-group.max    (max 1 (quotient len.row*.max count.row-groups.max)))
         (code*                (make-fxvector len.code*.max))
         (code*.group          (make-fxvector (* len.row-group.max count.columns)))
         (code*.dedup          (make-fxvector len.row-group.max))
         (parse/bt             (parser in count.columns input-batch-size code*))
         (row*                 (make-fxvector len.row*.max))
         (row*.buffer          (make-fxvector len.row*.max))
         (count*.row-group     (make-fxvector count.row-groups.max))
         (offset*.row-group    (make-fxvector count.row-groups.max))
         (buffer.out           (make-bytes row-group-size)))
    (define (row<? a b)
      ;; TODO: take column-permutation order into account
      (error "TODO: row<?"))
    (when (fx< count.columns 1) (error "too few columns" count.columns))
    (let loop ((i 0) (ord* (set)))
      (if (< i count.columns)
          (loop (+ i 1) (set-add ord* (fxvector-ref column-permutation i)))
          (unless (set-equal? (list->set (range count.columns)) ord*)
            (error "column permutation does not contain all column ordinals" column-permutation))))
    (when (< len.row*.max 2) (error "requested maximum size is too small" input-batch-size))
    (pretty-log '(building table) `(count.columns: ,count.columns)
                `(input-batch-size: ,input-batch-size)
                `(row-group-size: ,row-group-size))
    (let loop.table ((table-address* '()))
      (let*-values (((bt)                   (make-btree))
                    ((len.row* parsed-size) (performance-log '(parsing input batch) (parse/bt bt))))
        (verbose-log `(parsed ,len.row* rows))
        (if (eof-object? len.row*)
            (let* ((len.table* (length table-address*))
                   (table*     (make-fxvector len.table*)))
              (let loop ((i (unsafe-fx- len.table* 1)) (table-address* table-address*))
                (unsafe-fxvector-set! table* i (car table-address*))
                (when (unsafe-fx< 0 i) (loop (unsafe-fx- i 1) (cdr table-address*))))
              table*)
            (let* ((len.code*  (unsafe-fx* len.row* count.columns))
                   (count.bt   (btree-count bt))
                   (t*.global  (make-vector count.bt))
                   (code=>code (make-fxvector count.bt))
                   (code.new   0))
              (verbose-log `(found ,count.bt unique text values))
              (btree-enumerate
                bt
                (lambda (t code.old)
                  (unsafe-vector*-set! t*.global code.new t)
                  (unsafe-fxvector-set! code=>code code.old code.new)
                  (set! code.new (unsafe-fx+ code.new 1))))
              (let loop ((i 0))
                (when (unsafe-fx< i len.code*)
                  (let ((code.old (unsafe-fxvector-ref code* i)))
                    (unsafe-fxvector-set! code* i (unsafe-fxvector-ref code=>code code.old)))
                  (loop (unsafe-fx+ i 1))))
              (let loop ((row 0))
              ...
              ;; row-groups, but fewer rows per group, keeping each group's data size close to the
              ;; requested amount.  If instead we scaled the number of row-groups according to
              ;; len.row*, we would end up with fewer, atypically large row-groups, which is bad.
              ...
              (let* ((count.row-groups (max 1 (quotient (* count.row-groups.max parsed-size)
                ...
                (verbose-log `(preparing to encode ,count.row-groups row-groups)
                             ...
                             `(at file position ,fp.table-start))
                (define (encode-row-group!)
                  (let loop.col ((col 0) (start.col 0))
                    (verbose-log `(preparing to encode column ,col))
                    (let ((end.col (unsafe-fxmin (unsafe-fx+ start.col len.row-group) len.row*)))
                      (unsafe-fxvector-copy! code*.dedup 0 code*.group start.col end.col)
                      (let* ((len.dedup  (unsafe-fxvector-dedup-adjacent!
                                           code*.dedup 0 (unsafe-fx- end.col start.col)))
                             (t*.local   (make-vector len.dedup)))
                        (let loop ((code.new 0))
                          (when (unsafe-fx< code.new len.dedup)
                            (let* ((code.old (unsafe-fxvector-ref code*.dedup code.new))
                                   (t        (unsafe-vector*-ref t*.global code.old)))
                              (unsafe-vector*-set! t*.local code.new t)
                              (unsafe-fxvector-set! code=>code code.old code.new))
                            (loop (unsafe-fx+ code.new 1))))
                        (let loop ((i start.col))
                          (when (unsafe-fx< i end.col)
                            (let* ((code.old (unsafe-fxvector-ref code*.group i))
                                   (code.new (unsafe-fxvector-ref code=>code code.old)))
                              (unsafe-fxvector-set! code*.group i code.new))
                            (loop (unsafe-fx+ i 1))))
                        (let-values (((size encode) (performance-log
                                                      `((determining column encoding)
                          ...
                          (let ((size.encoded (performance-log `(encoding ,size bytes)
                                                               (encode buffer.out 0))))
                            (unless (unsafe-fx= size.encoded size)
                              (error "encoded size differs from requested size" size.encoded size)))
                          (write-bytes buffer.out out 0 size)
                          (when (unsafe-fx< (unsafe-fx+ col 1) count.columns)
                            (loop.col (unsafe-fx+ col 1) (unsafe-fx+ start.col len.row-group))))))))
                (let loop.group ((row 0) (row-group 0))
                  (let ((last-row.group
                          (unsafe-fx- (unsafe-fxmin (unsafe-fx+ len.row-group row) len.row*) 1)))
                    (verbose-log `(preparing to encode row-group ,row-group)
                                 `(containing rows ,row to ,(+ last-row.group 1)))
                    (unsafe-fxvector-set! count*.row-group row-group
                                          (unsafe-fx+ (unsafe-fx- last-row.group row) 1))
                    (let loop.row ((row row))
                      (let ((row-base (unsafe-fx* (unsafe-fxvector-ref row* row) count.columns)))
                        (let loop.col ((col.old 0) (target row))
                          (let* ((col  (unsafe-fxvector-ref column-permutation col.old))
                                 (code (unsafe-fxvector-ref code* (unsafe-fx+ row-base col))))
                            (unsafe-fxvector-set! code*.group target code))
                          (cond
                            ((unsafe-fx< (unsafe-fx+ col.old 1) count.columns)
                             (loop.col (unsafe-fx+ col.old 1) (unsafe-fx+ target len.row-group)))
                            ((unsafe-fx< row last-row.group) (loop.row (unsafe-fx+ row 1)))
                            (else
                              (encode-row-group!)
                              (unsafe-fxvector-set! offset*.row-group row-group
                                                    (unsafe-fx- (file-position out) fp.table-start))
                              (when (unsafe-fx< (unsafe-fx+ row 1) len.row*)
                                (loop.group (unsafe-fx+ row 1) (unsafe-fx+ row-group 1))))))))))
                (let ((fp.table-metadata-start (file-position out)))
                  (verbose-log '(preparing table metadata)
                               `(at file position ,fp.table-metadata-start))
                  (define (encode-metadata-nat* n* count)
                    (performance-log `(determining column encoding for ,count integers)
                                     (encode-int* n* 0 count)))
                  (let*-values
                    (((size.column-type* encode.column-type*)
                      (encode-metadata-nat* column-type* count.columns))
                     ((size.count*.row-group encode.count*.row-group)
                      (encode-metadata-nat* count*.row-group count.row-groups))
                     ((size.offset*.row-group encode.offset*.row-group)
                      (encode-metadata-nat* offset*.row-group count.row-groups)))
                    (let* ((data-size             (unsafe-fx- fp.table-metadata-start
                                                              fp.table-start))
                           (metadata-size         (unsafe-fx+ size.column-type*
                                                              size.count*.row-group
                                                              size.offset*.row-group))
                           (fp.table-metadata-end (unsafe-fx+ fp.table-metadata-start
                                                              metadata-size))
                           (bw.count.columns      (nat-min-byte-width count.columns))
                           (bw.count.row-groups   (nat-min-byte-width count.row-groups))
                           (bws.column&row-group  (unsafe-fxior (unsafe-fxlshift bw.count.columns 4)
                                                                bw.count.row-groups))
                           (bw.data-size          (nat-min-byte-width data-size))
                           ...
                           (size.footer           (unsafe-fx+ 3
                                                              ...
                                                              bw.data-size
                                                              bw.metadata-size))
                           (size.full             (unsafe-fx+ metadata-size size.footer)))
                      (verbose-log `(preparing table footer (,size.footer bytes))
                                   `((count.row-groups    ,count.row-groups)
                                     (count.columns       ,count.columns)
                                     (data-size           ,data-size)
                                     (metadata-size       ,metadata-size)
                                     (bw.count.columns    ,bw.count.columns)
                                     (bw.count.row-groups ,bw.count.row-groups)
                                     (bw.data-size        ,bw.data-size)
                                     (bw.metadata-size    ,bw.metadata-size)
                                     (tag.table           ,tag.table))
                                   `(at file position ,fp.table-metadata-end))
                      (when (unsafe-fx< (unsafe-bytes-length buffer.out) size.full)
                        (set! buffer.out (make-bytes size.full)))
                      (let ((size.encoded
                              (performance-log
                                `(encoding ,size.full bytes)
                                (let* ((pos 0)
                                       (pos (encode.column-type*      buffer.out pos))
                                       (pos (encode.count*.row-group  buffer.out pos))
                                       (pos (encode.offset*.row-group buffer.out pos))
                                       (pos (advance-unsafe-bytes-nat-set!/width
                                              bw.count.row-groups buffer.out pos count.row-groups))
                                       (pos (advance-unsafe-bytes-nat-set!/width
                                              bw.count.columns buffer.out pos count.columns))
                                       (pos (advance-unsafe-bytes-nat-set!/width
                                              bw.data-size buffer.out pos data-size))
                                       (pos (advance-unsafe-bytes-nat-set!/width
                                              bw.metadata-size buffer.out pos metadata-size))
                                       (pos (advance-unsafe-bytes-set! buffer.out pos
                                                                       bws.column&row-group))
                                       (pos (advance-unsafe-bytes-set! buffer.out pos
                                                                       bws.data&metadata))
                                       (pos (advance-unsafe-bytes-set! buffer.out pos tag.table)))
                                  pos))))
                        (unless (unsafe-fx= size.encoded size.full)
                          (error "encoded size differs from requested size" size.encoded size.full))
                        (write-bytes buffer.out out 0 size.full))))))
              (let ((address (file-position out)))
