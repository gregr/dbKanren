...
(define (column-group/key col)
  (let ((group/key* (column-group/key* col)))
    (lambda (key start end)
      (group/key* (vector key) start end))))
;; key start end -> (yield! key insertion-and-span.start span.end)
(define (column-sorted-group/key col)
  (let ((sorted-group/key* (column-sorted-group/key* col)))
    (lambda (key start end)
      (sorted-group/key* (vector key) start end))))
;; key? start end -> (yield! key span.start span.end)
(define (column-group/key? col)
  (let ((group (column-group col)))
    (lambda (key? start end)
      (let ((enum (group start end)))
        (lambda (yield!)
          (enum (lambda (x span.start span.end)
                  (when (key? x) (yield! x span.start span.end)))))))))
;; i* -> x*
...
(define (unsafe-bytes-prefix? x prefix)
  (let* ((len.x      (unsafe-bytes-length x))
         (len.prefix (unsafe-bytes-length prefix)))
    (and (unsafe-fx<= len.prefix len.x)
         (let loop ((i 0))
...
(define ((group/key*/single-value single-value x=x?) key* start end)
  (let ((len (unsafe-vector*-length key*)))
    (let loop ((i 0))
      (if (unsafe-fx< i len)
...
;; - the embedded bit width describes the size of each text code unit
...
(define (column:encoding.int bv start.bv count)
  (let-values (((col end) (column:encoding.int&end bv start.bv count)))
    col))
(define (column:encoding.text bv start.bv count)
  (let-values (((col end) (column:encoding.text&end bv start.bv count)))
    col))
...
(define (column:encoding.int:single-value single-value count)
  ...
  (define ((group start end) yield!) (yield! single-value start end))
      ...
      (let loop ((i 0))
        (when (unsafe-fx< i len)
          (let* ((key (unsafe-vector*-ref key* i))
            ...
            (let ((code (unsafe-fxquotient n z.delta)))
              (when (and (unsafe-fx<= start code) (unsafe-fx< code end))
                (yield! key code (if (unsafe-fx= (unsafe-fxremainder n z.delta) 0)
                                     (unsafe-fx+ code 1)
                                     code))))
            (loop (unsafe-fx+ i 1)))))))
...
(define ((column:encoding.X:dictionary/X x<x? x=x? column-type-id.description encoding-id.description)
  ...
  (let* ((ref.code               (column-ref col.code))
    ...
    (define (group/key* key* start end)
      (let ((i 0) (key*.encoded (make-vector (unsafe-vector*-length key*))))
        ((sorted-group/key*.dict key* 0 count.dict)
         (lambda (key start end)
           (unless (unsafe-fx= start end)
             (unsafe-vector*-set! key*.encoded i start)
             (set! i (unsafe-fx+ i 1)))))
        (if (unsafe-fx< 0 i)
            (let ((key*.encoded (vector-copy key*.encoded 0 i)))
              (group-map (group/key*.code key*.encoded start end) ref.dict))
            (lambda (yield!) (void)))))
...
(define ((column:encoding.X:run-length/X column-type-id.description encoding-id.description)
  ...
  (let* ((ref.pos               (column-ref               col.pos))
    ...
    (define (group/make-enum make-enum start end)
      (let* ((i.start      (unsafe-bisect-skip 0 (unsafe-fx+ count.run 1)
                                               (lambda (i) (unsafe-fx< (ref.pos i) start))))
             (i.end        (unsafe-bisect-skip i.start (unsafe-fx+ count.run 1)
                                               (lambda (i) (unsafe-fx< (ref.pos i) end))))
             (enum         (make-enum i.start i.end)))
        (lambda (yield!)
          (enum (lambda (x start.run end.run) (yield! x (unsafe-fxmax (ref.pos start.run) start)
                                                      (unsafe-fxmin (ref.pos end.run) end)))))))
    (define (group start end) (group/make-enum group.run start end))
    (define (group/key* key* start end)
      (group/make-enum (lambda (start end) (group/key*.run key* start end)) start end))
    (define (sorted-group/key* key* start end)
      (group/make-enum (lambda (start end) (sorted-group/key*.run key* start end)) start end))
    (controller:column (controller:missing-method column-type-id.description)
                       count describe ref group group/key* sorted-group/key*)))
...
(define (column:encoding.text:single-prefix prefix col.suffix count)
  ...
  (let* ((len.prefix               (unsafe-bytes-length prefix))
    ...
    (define (group start end)
      (let ((enum (group.suffix start end)))
        (lambda (yield!)
          (enum (lambda (suffix start end) (yield! (decode suffix) start end))))))
    (define (group/key* key* start end)
      (let ((enum (group/key*.suffix (encode* key*) start end)))
        (lambda (yield!)
          (enum (lambda (suffix start end) (yield! (decode suffix) start end))))))
    (define (sorted-group/key* key* start end)
      (let ((len          (unsafe-vector*-length key*))
            (key*.encoded (encode* key*)))
        (let ((enum (sorted-group/key*.suffix key*.encoded start end)))
          (lambda (yield!)
            (let loop ((i 0))
              (when (unsafe-fx< i len)
                (let ((key (unsafe-vector*-ref key* i)))
