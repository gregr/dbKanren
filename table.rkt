...
(define (table/vref vref key-col cols types row-count)
  (let table ((cols  cols)
              (types types)
    ...
    (and
      ...
      (let ()
        ...
        (define compare          (and (pair? types)
                                      (type->compare (car types))))
        ...
        (define (key-col? col)
          (cond ((and (not key-bound?) (equal? col key-col)) #t)
                ((equal? col (car cols))                     #f)
                (else (error "invalid fast column:" col))))
        (method-lambda
          ...
          ((>  col v)      (trim (if (key-col? col)
                                   (if (integer? v)
                                     (min (max start (+ v 1)) end) end)
                                   (skip<= v))
                                 end))
          ((>= col v)      (trim (if (key-col? col)
                                   (if (integer? v)
                                     (min (max start    v   ) end) end)
                                   (skip<  v))
                                 end))
          ((<  col v)      (trim start
                                 (if (key-col? col)
                                   (if (integer? v)
                                     (min (max start (- v 1)) end) start)
                                   (skip>= v))))
          ((<= col v)      (trim start
                                 (if (key-col? col)
                                   (if (integer? v)
                                     (min (max start    v   ) end) start)
                                   (skip>  v))))
          ((=  col v)      (if (key-col? col)
                             ...
                             (let* ((start.new (skip< v))
                                    (end.new   (bisect-next start.new end
                                                            (make-i<= v))))
                               (table (cdr cols) (cdr types) (cons col bound)
                                      (+ mask 1) start.new end.new)))))))))
