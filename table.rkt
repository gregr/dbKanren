...
(define (table ref types mask start end)
  ...
  (define (make-prefix<= prefix)
    (tuple<=? (list->vector (s-take (vector-length prefix) compares))))
  (define (ref* i) (vector-copy (ref i) mask))
  (define (make-i< prefix) (let ((prefix< (make-prefix< prefix)))
                             (lambda (i) (prefix< (ref* i) prefix))))
  (define (make-i<= prefix) (let ((prefix<= (make-prefix<= prefix)))
                              (lambda (i) (prefix<= (ref* i) prefix))))
  ;(define (make-i> prefix) (let ((prefix< (make-prefix< prefix)))
    ...
    ((stream)  (let loop ((i 0))
                 (thunk (if (= i (- end start)) '()
                          (cons (vector->list (ref* (+ start i)))
                                (loop (+ i 1)))))))
    ((find< prefix)  (bisect start end (make-i< prefix)))
    ((find<= prefix) (bisect start end (make-i<= prefix)))
    ((drop< prefix)  (table ref types mask
                            (bisect-next start end (make-i< prefix)) end))
    ((drop<= prefix) (table ref types mask
                            (bisect-next start end (make-i<= prefix)) end))
    ((take<= prefix) (table ref types mask
                            start (bisect-next start end (make-i<= prefix))))
    ;; TODO: > >= variants: take>= drop> drop>=
    ;((drop> prefix)  (table ref types mask
                            ;start (bisect-prev start end (make-i> prefix))))
    ((take count) (table ref types mask start           (+ count start)))
    ((drop count) (table ref types mask (+ count start) end))))
...
(define (table/port/offsets table.offsets types in)
  ...
  (define (ref i)
    ...
    (decode in type))
  (table ref types 0 0 (table.offsets 'length)))
...
(define (table/vector types v)
  (table (lambda (i) (vector-ref v i)) types 0 0 (vector-length v)))
  ...
  (define t.value
    (case retrieval-type
      ((disk) (define in.value (open-input-file fname.value))
              (if offset-type
                (table/port/offsets (table/port len `(,offset-type)
                                                (open-input-file fname.offset))
                                    column-types in.value)
