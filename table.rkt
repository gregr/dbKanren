  ...
  (define index-ts
    (let* ((ss.sources (generate-temporaries primary-source-names))
      ...
      (map (lambda (column-names)
           ...
           (cdr table-layouts))))
  (list (hash-set info 'missing-data? (not (hash-ref info 'source-vector #f)))
...
(define (materialization . pargs)
  ...
  (cond (src:vector
          ...
          (materialization/vector src:vector sort? dedup? kwargs.1))
        (path (when (or src:path.0 src:stream)
                (materialize-relation! path path.dir path.metadata metadata kwargs.1))
...
(define (materialize-relation! path path.dir path.metadata info kwargs)
  ...
  (define format.in       (hash-ref kwargs 'source-file-format  #f))
  (define header.in       (hash-ref kwargs 'source-file-header  '()))
  (define names.in        (hash-ref kwargs 'source-file-columns (remove key-name attribute-names)))
  ...
  (define (materialize-stream source-info stream)
    (let ((mat (materializer path.dir source-info
                             ...
                             table-layouts)))
      (define count 0)
      ;; TODO: eliminate this, let the user map if necessary
      (define arrange (list-arranger names.in (car table-layouts)))
      (time (s-each (lambda (x)
                      (when (= 0 (remainder count threshold))
                        (logf "ingested ~s rows\n" count))
                      (mat 'put! (arrange x))
